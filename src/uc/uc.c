...
void UC_tick(UC *uc, ULA *ula, BancoRegistradores *banco, Barramento *barramento, Memoria *memoria) {
    ...
    if (uc->pipeline->f_busca_operandos && !uc->pipeline->f_executar && !pipeline_checar_dep(uc->pipeline, p2_MAR, STOR)) {
    ...
    if (uc->pipeline->f_buscar_instrucao && !uc->pipeline->f_decodificar && !pipeline_checar_dep(uc->pipeline, rPC_read(banco), STOR_LR)) {
        PRINT("-- Busca de Instrucoes --");
        PRINT("rPC: %ld", banco->rPC);
        pipeline_buscar_instrucao(uc->pipeline, &p1_MBR, banco, barramento, memoria);
    }

    if (uc->pipeline->flags & STOR_EXECUTADO) {
        pipeline_pop_dep(uc->pipeline);
        uc->pipeline->flags &= ~STOR_EXECUTADO;
    }
}
